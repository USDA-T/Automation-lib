...
public class CommonApplicationMethods {
	...
	// WebElement aa= find_Element(WebDriver webDriver, String locator_Type, String
	// locator_Value);
	// 3. Optional Find element with Locator -
	// WebElement aa= find_Element(WebDriver webDriver, String locator_Yaml, Boolean
	// true);
	...
	public static List<WebElement> find_Elements(WebDriver webdriver, String type_Locator, String value_Locator)
	...
	public static List<WebElement> find_Elements(WebDriver webdriver, String locator_Yaml, Boolean optional_Check)
	...
	public static WebElement find_Element(WebDriver webdriver, String locator_Yaml, Boolean check_Optional)
			throws Exception /* Optional */
	{
		Map locator = getLocator(locator_Yaml);
		String loc = locator.get("Locator").toString();
	...
	public static void verify_Text(WebDriver webdriver, String locator_Yaml, String text_to_Verify) throws Exception {
		...
		assertEquals(find_Element(webdriver, loc.get("Locator").toString(), loc.get("Value").toString()).getText(),
				text_to_Verify);
	}

	public static void verify_Text(WebDriver webdriver, String loc_Yml, String val_Yml, String text_to_Verify)
			throws Exception {
	...
	public static void double_Click_Element(WebDriver webDriver, String type_Locator, String value_Locator)
			throws Exception {

		WebElement element_Name = find_Element(webDriver, type_Locator, value_Locator);
		// ((JavascriptExecutor)
		// webDriver).executeScript("arguments[0].scrollIntoView(true);", element_Name);
	...
	public static void click_Element(WebDriver webDriver, String locator_Yaml) throws Exception {
		try {
			...
			for (int i = 0; i < 9900000; i++) {
				...
				// display(get_Element.getText()); //
				// display(get_Element.getAttribute("innerHTML")); //
				// //Debug
				Dimension get_Element_D = get_Element.getSize();
				if (get_Element_D.getWidth() > 0 && get_Element_D.getHeight() > 0 && get_Element.isEnabled()) {
					// ((JavascriptExecutor)
					// webDriver).executeScript("arguments[0].scrollIntoView(true);", get_Element);
					get_Element.click();
					return;
				}
	...
	public static String returnOrganization_Id(String duns_Number) throws Exception {
		...
		try {
			...
			Thread.sleep(3000); /*
								 * DEEPA: Sleep is needed here since we are querying SQL, and its too fast See
								 * below Start
								 */
	...
	public static void search_Cases_Duns_Number_Table(WebDriver webDriver, String search_Text) throws Exception {
		setText_Element(webDriver, "SBA_CaseTable_Search", search_Text);
	...
	public static void navigationMenuClick(WebDriver webDriver, String which_Button) throws Exception {
		...
		/*
		 * WebElement check = find_Element(webDriver, "Navigation_Present_Or_Not",
		 * false); if (check == null){ navigationBarClick( webDriver, which_Button);
		 * return; }
		 */
		...
		case "PROGRAMS":
			...
			break;
		case "DASHBOARD":
			// Example Documentation: Use _02 instead for newer elements. Older elements
			// have been retained.
